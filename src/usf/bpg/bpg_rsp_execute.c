...
int bpg_rsp_copy_pdt_to_ctx(bpg_rsp_t rsp, logic_context_t op_context, struct basepkg * pkg, error_monitor_t em);
...
int bpg_rsp_execute(gd_dp_req_t req, void * ctx, error_monitor_t em) {
    ...
    if (bpg_rsp_copy_pdt_to_ctx(bpg_rsp, op_context, pkg, em) != 0) {
...
static int bpg_rsp_copy_main_to_ctx(bpg_manage_t mgr, logic_context_t op_context, struct basepkg * pkg, error_monitor_t em) {
    ...
    if (cmd_entry_idx < 0) {
        if (logic_context_flag_is_enable(op_context, logic_context_flag_debug)) {
            CPE_INFO(
                em, "bpg_rsp_execute: copy_pdt_to_ctx: request meta %s have no associate info of cmd %d!",
        ...
        if (data_meta == NULL) {
            CPE_ERROR(
                em, "bpg_rsp_execute: copy_pdt_to_ctx: %s.%s have no associate meta!",
        ...
        if (data == NULL) {
            CPE_ERROR(
                em, "bpg_rsp_execute: copy_pdt_to_ctx: %s create data fail, capacity=%d!",
        ...
        {
            CPE_ERROR(
                em, "bpg_rsp_execute: copy_pdt_to_ctx: %s decode data fail, input len is %d, output len is %d!",
...
static int bpg_rsp_copy_append_to_ctx(bpg_manage_t mgr, logic_context_t op_context, struct basepkg * pkg, error_monitor_t em) {
    ...
    if (pkg->head.appendInfoCount > (sizeof(pkg->head.appendInfos) / sizeof(pkg->head.appendInfos[0]))) {
        CPE_ERROR(em, "bpg_rsp_execute: copy_pdt_to_ctx: append info count %d overflow!", pkg->head.appendInfoCount);
        return -1;
    }
    ...
    for(i = 0; i < pkg->head.appendInfoCount; ++i) {
        ...
        if (buf_end > pkg->head.bodytotallen) {
            CPE_ERROR(
                em, "bpg_rsp_execute: copy_pdt_to_ctx: append %d: buf overflow, total len is %d, curent is %d!",
        ...
        if (data_meta == NULL) {
            CPE_ERROR(
                em, "bpg_rsp_execute: copy_pdt_to_ctx: append %d: meta of id %d not exist in lib!",
                i, append->id);
            return -1;
        }
        ...
        if (data == NULL) {
            CPE_ERROR(
                em, "bpg_rsp_execute: copy_pdt_to_ctx: append %d: %s create data fail, capacity=%d!",
        ...
        {
            CPE_ERROR(
                em, "bpg_rsp_execute: copy_pdt_to_ctx: append %d: %s decode data fail, input len is %d, output len is %d!",
...
int bpg_rsp_copy_pdt_to_ctx(bpg_rsp_t rsp, logic_context_t op_context, struct basepkg * pkg, error_monitor_t em) {
    ...
    if (mgr->m_metalib == NULL) {
        CPE_ERROR(em, "bpg_rsp_execute: copy_pdt_to_ctx: metalib is not set!");
        return -1;
    }

    if (mgr->m_request_meta == NULL) {
        CPE_ERROR(em, "bpg_rsp_execute: copy_pdt_to_ctx: request meta is not set!");
        return -1;
    }

    if (mgr->m_cvt_decode == NULL) {
        CPE_ERROR(em, "bpg_rsp_execute: copy_pdt_to_ctx: decode is not set!");
        return -1;
    }
