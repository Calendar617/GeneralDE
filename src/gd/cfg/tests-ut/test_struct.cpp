...
TEST_F(CfgTest, struct_add_basic) {
...
TEST_F(CfgTest, struct_add_not_from_struct) {
...
TEST_F(CfgTest, struct_find_empty) {
    EXPECT_FALSE(gd_cfg_struct_find_cfg(m_root, "a"));
}

TEST_F(CfgTest, struct_find_success) {
    gd_cfg_t s = gd_cfg_struct_add_struct(m_root, "a");
    ASSERT_TRUE(s);
...
TEST_F(CfgTest, struct_find_not_from_struct) {
    EXPECT_FALSE(gd_cfg_struct_find_cfg(gd_cfg_struct_add_seq(m_root, "a"), ""));
}

TEST_F(CfgTest, struct_free_child) {
    gd_cfg_t s = gd_cfg_struct_add_struct(m_root, "a");
    ASSERT_TRUE(s);
